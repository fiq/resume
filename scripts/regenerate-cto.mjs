...
const sourcePath = path.resolve(repoRoot, "resume_content_cto.md");
const outputPath = path.resolve(repoRoot, "cv", "cto.html");
...
const STATIC_CONTACT = {
  ...
  alternateCvHref: "https://fiq.github.io/resume/cv/index.html",
  alternateCvLabel: "Hands-on Tech Experience CV"
};
...
function getRequiredSection(byTitle, title) {
...
function renderEarlierCareer(section) {
  ...
  return `          <section class="content" id="earlier-career">
            <h2 class="section-title">Earlier Career Highlights</h2>
...
${entries
  .map((entryLines) => {
    ...
    const entry = { title, institute };
    const date = STATIC_EDUCATION_DATES.get(entry.title);
    return `              <div class="education-item">
                <h3 class="education-title">
                  ${escapeHtml(entry.title)}
                </h3>
${entry.institute ? `                <div class="education-institute">
                  ${escapeHtml(entry.institute)}
                </div>
...
function buildHtml(sections) {
  ...
  for (const title of REQUIRED_SECTION_TITLES) {
    getRequiredSection(byTitle, title);
  }

  const contact = parseContactSection(findContactSection(sections));
  const summary = parseBlocks(getRequiredSection(byTitle, "Summary").lines);
  ...
  const leadershipFocus = parseBlocks(getRequiredSection(byTitle, "Engineering Leadership Focus").lines);
  const community = parseBlocks(getRequiredSection(byTitle, "Community & Industry Contributions").lines);
  const technicalFoundations = parseBlocks(getRequiredSection(byTitle, "Technical Foundations").lines);
        ...
        >
          <div class="info-left header-content">
            ...
            <div class="intro-title mb-2 ps-4">
              ${renderInlineMarkdown(getSingleParagraphText(getRequiredSection(byTitle, "Header Tagline"), "Header Tagline"))}
            </div>
            <div class="intro-credibility ps-4">
              ${renderInlineMarkdown(getSingleParagraphText(getRequiredSection(byTitle, "Header Credibility Line"), "Header Credibility Line"))}
            </div>
...
${renderEarlierCareer(byTitle.get("Earlier Career Highlights"))}

${renderEducation(getRequiredSection(byTitle, "Education"))}
        </div>

        <aside class="resume-sidebar">
${renderSimpleSection("Engineering Leadership Focus", leadershipFocus, {
  id: "leadership-focus",
  sectionClasses: "content sidebar-card mt-2",
  listClassName: "role-bullets compact-list"
})}

${renderSimpleSection("Community & Industry Contributions", community, {
  id: "community",
...
${renderSimpleSection("Technical Foundations", technicalFoundations, {
...
async function main() {
  ...
  if (sections.length === 0) {
    throw new Error("No sections found in resume_content_cto.md");
  }
